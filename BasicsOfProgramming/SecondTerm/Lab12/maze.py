...
class Maze:
    ...
    def find_path(self):
        ...
        while not stack.is_empty():
            current_sell = stack.pop()
            self._mark_path(current_sell.row, current_sell.col)
            if self._exit_found(current_sell.row, current_sell.col):
                return True
            next_cells = [
                _CellPosition(current_sell.row, current_sell.col-1),
                _CellPosition(current_sell.row+1, current_sell.col),
                _CellPosition(current_sell.row, current_sell.col+1),
                _CellPosition(current_sell.row-1, current_sell.col)
            ]
            prev_cells = [
            ...
            for next_cell in next_cells:
                if self._valid_move(next_cell.row, next_cell.col):
                    ...
                    continue
            if not has_valid:
                self._mark_tried(current_sell.row, current_sell.col)
                for prev_cell in prev_cells:
                    if self._get_cell_value(prev_cell) == self.PATH_TOKEN:
