class Book:
    def __init__(self, name, author, pages, cur_pg=1, mark=None) -> None:
        self.name = name
        self.author = author
        self.pages = pages
        self.current_page = cur_pg
        self.bookmarked_page = mark

    def __repr__(self) -> str:
        pg_str = "1 page" if self.pages == 1 else f"{self.pages} pages"
        if self.bookmarked_page == None:
            return f"Book<{self.name} by {self.author}: {pg_str}, currently on page {self.current_page}>"
        else:
            return f"Book<{self.name} by {self.author}: {pg_str}, currently on page {self.current_page}, page {self.bookmarked_page} bookmarked>"

    def turn_page(self, amount_of_pages):
        self.current_page = self.current_page + amount_of_pages
    ...
    def __eq__(self, __o: object) -> bool:
        if not isinstance(__o, Book):
            ...
            for i in range(n):
                if getattr(self, prop_names1[i]) != getattr(__o, prop_names2[i]):
                    return False
